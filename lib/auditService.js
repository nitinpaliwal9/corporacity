...
class AuditService {
  ...
  async getAuditLogs(companyId, options = {}) {
    ...
    try {
      // First try with user profile join
      let query = this.supabase
        .from('corp_audit_logs')
        .select(`
          *,
          user:corp_profiles!user_id(full_name, email)
        `)
      ...
      if (error) {
        ...
        if (error.code === 'PGRST200' || error.message.includes('relationship')) {
          console.warn('Audit logs table or relationship not found, falling back to basic query');
          ...
          if (resourceType) fallbackQuery = fallbackQuery.eq('resource_type', resourceType);
    ...
    try {
      let query = this.supabase
        ...
        .select(`
          *,
          user:corp_profiles!user_id(full_name, email),
          resolved_by_user:corp_profiles!resolved_by(full_name, email)
        `)
        .eq('company_id', companyId)
        .order('created_at', { ascending: false });
      ...
      if (error) {
        // If the join fails, try without the join
        if (error.code === 'PGRST200' || error.message.includes('relationship')) {
          console.warn('Security events table or relationship not found, falling back to basic query');
          ...
          if (isResolved !== null) fallbackQuery = fallbackQuery.eq('is_resolved', isResolved);
          if (startDate) fallbackQuery = fallbackQuery.gte('created_at', startDate);
          if (endDate) fallbackQuery = fallbackQuery.lte('created_at', endDate);
