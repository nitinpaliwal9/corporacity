-- SAFE RLS POLICIES - NO CIRCULAR REFERENCES
-- This version completely eliminates circular dependencies
...
-- ==============================================
-- SAFE POLICIES - NO CIRCULAR REFERENCES
-- ==============================================

-- 1. COMPANIES - Only reference the companies table itself
...
-- 2. MEMBERSHIPS - Only reference the memberships table itself
-- Users can view their own memberships
CREATE POLICY "Users can view own memberships" ON corp_memberships
  FOR SELECT USING (user_id = auth.uid());

-- Users can create memberships (we'll handle company owner validation in the application)
...
-- 3. JOIN REQUESTS - Only reference the join_requests table itself
...
-- 4. STATUSES - Only reference the statuses table itself
...
-- 5. PROFILES - Only reference the profiles table itself
