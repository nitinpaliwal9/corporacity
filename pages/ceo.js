...
export default function CeoDashboard() {
  ...
  useEffect(() => {
    ...
    supabase.from('statuses').select('*, users ( id, email )').order('timestamp', { ascending: false }).limit(50).then(({ data }) => {
      setFeed(data || [])
    })

    // fetch join requests
    supabase.from('join_requests').select('*, users ( id, email )').order('created_at', { ascending: false }).then(({ data }) => {
      setRequests(data || [])
    })
  ...
  const approve = async (req) => {
    ...
    await supabase.from('memberships').insert([{ user_id: req.user_id, company_id: req.company_id }])
        ...
        <ul>
          {requests.map(r => (
            <li key={r.id} className='border p-2 mb-2 flex justify-between items-center'>
              <div>{r.users?.email || r.user_id}</div>
        ...
        <ul>
          {feed.map(item => (
            <li key={item.id} className='border p-2 mb-2'>
              ...
              <div><strong>{item.user_id}</strong> — {item.type} {item.message}</div>
            </li>
          ))}
        </ul>
      </section>
